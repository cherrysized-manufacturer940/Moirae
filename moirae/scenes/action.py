...
from moirae.schema import ActionScene
from moirae.scenes import (
    PlaybackContext, SceneTiming, _BOLD, _DIM, _RST,
    register,
)
from moirae.scenes.conversation import type_text
...
def _action_camera(scene: ActionScene, ctx: PlaybackContext):
    ...
    if scene.auto_y:
        scene_timing.markers["_auto_y"] = 1.0
        scene_timing.markers["_cursor_camera"] = ctx.cursor_row
...
def _action_banner(scene: ActionScene, ctx: PlaybackContext):
    ...
    # Estimate banner height: empty + logo(6) + empty + panel(~21) ≈ 29 lines
    ctx.newline(29)
