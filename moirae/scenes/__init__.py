...
from moirae.skin_engine import SkinConfig, load_skin
...
class PlaybackContext:
    ...
    cursor_row: int = 0   # Current terminal row (incremented on newlines)
    total_rows: int = 80  # Total terminal rows (set by pipeline)
    ...
    def mark(self, scene_timing: SceneTiming, name: str):
        ...
        scene_timing.markers[f"_cursor_{name}"] = self.cursor_row

    def newline(self, count: int = 1):
        """Track cursor moving down by count rows."""
        self.cursor_row = min(self.cursor_row + count, self.total_rows)
...
def dispatch(scene: Any, ctx: PlaybackContext) -> None:
    ...
    from moirae.schema import ActionScene, ConversationScene
